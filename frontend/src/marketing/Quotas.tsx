...
function fmtRate(rpm: number): string {
  if (rpm >= 1000)
    return `${(rpm / 1000).toFixed(rpm % 1000 === 0 ? 0 : 1)}k / min`;
  return `${rpm} / min`;
}
...
function fmtQuota(quota: number): string {
  ...
  if (quota >= 1_000_000)
    return `${(quota / 1_000_000).toFixed(quota % 1_000_000 === 0 ? 0 : 1)}M / mo`;
  if (quota >= 1_000)
    return `${(quota / 1_000).toFixed(quota % 1_000 === 0 ? 0 : 1)}k / mo`;
  return `${quota} / mo`;
}
      ...
      <div className="quotas-page">
                ...
                >
                  ...
                  <footer>
                    {isCurrent ? (
                      ...
                      <Link to="/enterprise" className="quotas-cta">
                        {copy.cta} →
                      </Link>
                    ) : (
                      <Link
        ...
        <section className="quotas-fineprint">
          ...
          <ul>
            ...
            <li>
              The cycle is the <strong>calendar month</strong> in UTC. Counters
              reset at <code>00:00 UTC</code> on the first of every month,
              separate from your Stripe billing-cycle anniversary.
            </li>
            <li>
