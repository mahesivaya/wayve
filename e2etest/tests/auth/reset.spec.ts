...
import { purgeAllEmail, waitForEmailTo } from "../../fixtures/mailpit";

// Forgot-password → reset flow. Verifies:
//   1. Forgot endpoint mails a token via Mailpit
//   2. /reset-password accepts the token and a fresh password
//   3. The OLD password no longer works after reset
//
// Requires Mailpit to be reachable at E2E_MAILPIT_API. If it isn't,
...
test.describe("password reset", () => {
  test.beforeAll(async () => {
    try {
      ...
      test.skip(true, "Mailpit not reachable — skipping reset suite");
    }
  });
  ...
  // to Mailpit (`SMTP_HOST=mailpit SMTP_PORT=1025`), the forgot-password
  // mail is delivered to Gmail which silently drops @test.local emails
  // and Mailpit stays empty. Skip the round-trip until SMTP is pointed
  // at Mailpit for E2E runs. Same convention CLAUDE.md describes for
  // backend tests: Mailpit-dependent tests skip themselves when the
  ...
  test("full forgot → email → reset → login round-trip", async ({ page }) => {
    ...
    // Mailpit should have a message addressed to this user. If not, the
    // backend's SMTP host probably isn't pointing at Mailpit — skip
    ...
    try {
      ...
      test.skip(
        true,
        "Reset mail not delivered to Mailpit — backend SMTP probably points at smtp.gmail.com. " +
          "To exercise this test, set SMTP_HOST=mailpit SMTP_PORT=1025 on the backend container.",
      );
      return;
    }
