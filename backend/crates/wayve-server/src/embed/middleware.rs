...
/// Defined in `wayve_security::embed::middleware` so the central auth
/// chokepoint `wayve_security::jwt::get_user_id_from_request` can look
/// it up out of `request.extensions()` by `TypeId`. Re-exported here
/// so the rest of the server keeps its existing import path
/// (`embed::middleware::EmbedPrincipal`) intact.
